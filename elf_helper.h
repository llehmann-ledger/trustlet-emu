...
typedef int32_t  Elf32_Xoff;
typedef int64_t  Elf64_Xoff;
...
typedef struct
{
  ...
  Elf_Off	r_info;			/* Relocation type and symbol index */
} Elf_Rel;
...
typedef struct
{
  ...
  Elf_Off	p_align;		/* Segment alignment */
} Elf_Phdr;
#else
typedef struct
{
...
typedef struct
{
  Elf_Xoff	d_tag;			/* Dynamic entry type */
  Elf_Addr d_val;		/* Integer or Address value */
} Elf_Dyn;
...
typedef struct
{
  Elf_Word	vda_name;		/* Version or dependency names */
  Elf_Word	vda_next;		/* Offset in bytes to next verdaux
					   entry */
} Elf_Verdaux;
...
typedef struct
{
  Elf_Half	vn_version;		/* Version of structure */
  Elf_Half	vn_cnt;			/* Number of associated aux entries */
  Elf_Word	vn_file;		/* Offset of filename for this
					   dependency */
  Elf_Word	vn_aux;			/* Offset in bytes to vernaux array */
  Elf_Word	vn_next;		/* Offset in bytes to next verneed
					   entry */
} Elf_Verneed;
...
typedef struct
{
  Elf_Word	vna_hash;		/* Hash value of dependency name */
  Elf_Half	vna_flags;		/* Dependency specific information */
  Elf_Half	vna_other;		/* Unused */
  Elf_Word	vna_name;		/* Dependency name string offset */
  Elf_Word	vna_next;		/* Offset in bytes to next vernaux
					   entry */
} Elf_Vernaux;
...
typedef struct
{
  Elf_Off a_type;		/* Entry type */
  Elf_Off a_val;		/* Integer value */
} Elf_auxv_t;
...
typedef struct
{
  Elf_Xword m_value;		/* Symbol value.  */
  Elf_Off m_info;		/* Size and index.  */
  Elf_Off m_poffset;		/* Symbol offset.  */
  Elf_Half m_repeat;		/* Repeat count.  */
  Elf_Half m_stride;		/* Stride info.  */
} Elf_Move;

/* Macro to construct move records.  */
#define ELF_M_SYM(info)	((info) >> 8)
#define ELF_M_SIZE(info)	((unsigned char) (info))
#define ELF_M_INFO(sym, size)	(((sym) << 8) + (unsigned char) (size))
